// targetTP in pips (e.g., 50 pips)
input int targetTPPips = 30;
...
void OnTimer()
{
    ...
    {
        // Check total orders
        if(CloseOrders(OP_SELL)) {
            // Convert targetTP from pips to price difference
            double tpPriceDifference = targetTPPips * Point * 10;
            ModifyOrders(OP_BUY, Ask + tpPriceDifference);
        } else {
            Print("Cannot close SELL order!!!");
            return;
        }
    }
}
