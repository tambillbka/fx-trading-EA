...
void executeBuyOrder(double orderLots) {
    if (tradingMode == TWO_WAYS || (tradingMode == ONE_WAY && (executedOrder.getOrderType() == OP_BUY || executedOrder.getOrderType() == NO_ORDER))) {
        ...
        OpenOrder(OP_BUY, orderLots, buyPrice, buyPrice - (targetStopLoss * Point * 10), buyPrice + (targetTakeProfit * Point * 10), "Buy with lots: " + DoubleToString(orderLots));

        // Update executed order
        executedOrder.setOrderType(OP_BUY);
        executedOrder.setTakeProfit(buyPrice + (targetTakeProfit * Point * 10));
        executedOrder.setStopLoss(buyPrice - (targetStopLoss * Point * 10));
...
void executeSellOrder(double orderLots) {
    if (tradingMode == TWO_WAYS || (tradingMode == ONE_WAY && (executedOrder.getOrderType() == OP_SELL || executedOrder.getOrderType() == NO_ORDER))) {
        ...
        OpenOrder(OP_SELL, orderLots, sellPrice, sellPrice + (targetStopLoss * Point * 10), sellPrice - (targetTakeProfit * Point * 10), "Sell with lots: " + DoubleToString(orderLots));
